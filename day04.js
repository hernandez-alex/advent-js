...
function fitsInOneBox(boxes) {
  const boxesS = boxes.slice();
  boxesS.sort((a, b) => a.l - b.l);
  for (let i = 1; i < boxesS.length; i++) {
    if (!(boxesS[i].l > boxesS[i - 1].l)) return false;
    if (!(boxesS[i].w > boxesS[i - 1].w)) return false;
    if (!(boxesS[i].h > boxesS[i - 1].h)) return false;
  }
  return true;
}
