...
function getCompleted(part, total) {
  ...
  let partSec = +partArr[0] * 60 * 60 + +partArr[1] * 60 + +partArr[2];

  const totalArr = total.split(":");
  let totalSec = +totalArr[0] * 60 * 60 + +totalArr[1] * 60 + +totalArr[2];

  if (totalSec % partSec === 0) {
    return `1/${totalSec / partSec}`;
  }

  for (let i = 2; i <= Math.floor(totalSec / 2) + 1; i++) {
    while (partSec % i === 0 && totalSec % i === 0) {
      partSec = Math.floor(partSec / i);
      totalSec = Math.floor(totalSec / i);
    }
  }

  return `${partSec}/${totalSec}`;
}

console.log(getCompleted("03:30:30", "05:50:50")); // '1/3'
